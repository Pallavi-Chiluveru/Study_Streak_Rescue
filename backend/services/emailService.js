...
const configurationError = () => new Error('Email service configuration is incomplete.');
...
const initializeEmailService = async () => {
  ...
  if (process.env.NODE_ENV !== 'production') return verifyEmailService();
  return true;
};
...
const sendPasswordLinkEmail = async (email, rawToken, reset = false) => {
  ...
  const result = await getTransporter().sendMail({
    from: process.env.EMAIL_FROM, to: email, subject: `${action} your Study Streak Rescue password`,
...
exports.sendTestEmail = async () => {
  ...
  const result = await getTransporter().sendMail({
    from: process.env.EMAIL_FROM, to: process.env.EMAIL_USER,
